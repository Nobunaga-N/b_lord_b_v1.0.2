...
Шаблоны загружаются автоматически из папки:
...
Эмулятор: ID=5, порт=5564 (переопределить: --id / --port)

Версия: 1.2 — denom через OCR вместо template matching
"""
...
# Шаблоны (только типы — denom теперь через OCR)
...
# Зона номинала — текст "1m", "15m", "1h" и т.д. в верхнем левом углу ячейки
DENOM_ZONE = {
    'x1': 22,  'y1': 0,
    'x2': 91,  'y2': 26,
}
...
def try_parse_denom(text: str) -> Optional[str]:
    """
    ...
    Фиксы:
      - Кириллические замены: 'т' → 'm', 'п' → 'h', 'м' → 'm'
      - Латинские путаницы: 'М' → 'm', 'H' → 'h', 'D' → 'd'
      - Удаление пробелов: "1 5m" → "15m"
      - Удаление шума до/после паттерна
    """
    ...
    # Фикс цифр: OCR путает 'г'→1, 'б'→6, 'з'→3 и т.д.
    DIGIT_FIXES = {
        'г': '1', 'i': '1', 'l': '1', '|': '1',
        'б': '6', 'в': '8', 'з': '3', 'о': '0',
        'ч': '4', 'ь': '6', 'э': '9',
...
def ocr_denomination(crop: np.ndarray, ocr: OCREngine,
    ...
    if len(elements) > 1:
        ...
        denom = try_parse_denom(full_text)
        if denom is not None:
            logger.debug(
...
def mode_extract(emulator: dict, ocr: OCREngine):
    ...
    - cell_rX_cY_denom_ocr.png — OCR-input для denom
    ...
    for row in range(len(GRID_ROWS)):
        for col in range(len(GRID_COLS)):
            ...
            if denom_crop.size > 0:
                ...
                # OCR input для denom
                denom_enlarged = cv2.resize(
                    denom_crop, None, fx=DENOM_SCALE, fy=DENOM_SCALE,
                    interpolation=cv2.INTER_CUBIC
                )
                denom_padded = cv2.copyMakeBorder(
                    denom_enlarged,
                    DENOM_PADDING, DENOM_PADDING,
                    DENOM_PADDING, DENOM_PADDING,
                    cv2.BORDER_CONSTANT, value=(0, 0, 0)
                )
                cv2.imwrite(
                    str(DEBUG_DIR / f"{ts}_{label}_denom_ocr.png"),
                    denom_padded
                )
            ...
            # OCR номинала (для отладки)
            denom_val = ocr_denomination(denom_crop, ocr, label)
            ...
            denom_text = denom_val if denom_val is not None else "?"
            cv2.putText(annotated, f"{denom_text}={qty_text}",
                        (x1 + 5, y2 - 5),
            ...
            logger.info(
                ...
                f"denom_ocr={denom_val} qty_ocr={qty_val}"
            )
    ...
    logger.info(
        ...
        f"{TYPE_TEMPLATE_DIR}/\n"
        f"   3. Проверьте *_denom_ocr.png — OCR теперь читает denom\n"
...
def parse_screen(screenshot: np.ndarray, ocr: OCREngine,
    ...
    for row in range(len(GRID_ROWS)):
        for col in range(len(GRID_COLS)):
            ...
            # ── 2. OCR НОМИНАЛ ──
            denom_crop = crop_zone(cell, DENOM_ZONE)
            denom = ocr_denomination(denom_crop, ocr, label)
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("ТЕСТ ПАРСИНГА РЮКЗАКА v1.2")
