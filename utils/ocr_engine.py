...
class OCREngine:
    ...
    def __init__(self, lang: str = 'ru', force_cpu: bool = False):
        ...
        try:
            ...
            # Создаем OCR pipeline с явным указанием языка
            # Для русского используем славянскую модель распознавания
            if lang == 'ru':
                # Пытаемся использовать славянскую модель
                try:
                    self.pipeline = create_pipeline(
                        pipeline="OCR",
                        device=device,
                        det_model="PP-OCRv5_server_det",
                        rec_model="eslav_PP-OCRv5_mobile_rec"  # Славянская модель
                    )
                    logger.success(f"✅ OCR инициализирован (язык: {lang}, модель: eslav, устройство: {device})")
                except Exception as e:
                    logger.warning(f"⚠️ Не удалось загрузить славянскую модель: {e}")
                    logger.info("🔄 Использую стандартную модель с параметром language...")
                    # Fallback на стандартную модель с языком
                    self.pipeline = create_pipeline(
                        pipeline="OCR",
                        device=device,
                        language="rus"  # Попробуем параметр language
                    )
                    logger.success(f"✅ OCR инициализирован (язык: rus, устройство: {device})")
            else:
                # Для других языков используем стандартную модель
                self.pipeline = create_pipeline(
                    pipeline="OCR",
                    device=device
                )
